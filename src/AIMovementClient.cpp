...
void AIMovementClient::makeAIMovementPlans(const hexGrid &grid, std::vector<unit> &units, std::map<int, std::vector<int> > &movementPlans) {

    //Count up the patrol locations which are not currently being patrolled
    //I need to use an std::vector, because I need to be able to draw a random element
    ...
    //Convert vacent patrol positions to a vector, so we can efficiently use random access
    ...
    for (int unitId = 0; unitId < units.size(); ++unitId) {
        ...
        if (!u.isFriendly()) {
            ...
            if (patrol != -1) {
                ...
                auto fullPath =grid.findPath(origin,patrol,obstructed);
                ...
                if (movementPlans.contains(unitId)) {
                    movementPlans.at(unitId)=limitedPath;
                }
                else
                    movementPlans.insert(std::make_pair(unitId,limitedPath));
